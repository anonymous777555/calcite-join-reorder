...
import org.apache.calcite.rel.logical.LogicalNullifyStar;
...
public class RelFactories {
  ...
  public static final NullifyStarFactory DEFAULT_NULLIFY_STAR_FACTORY =
      new NullifyStarFactoryImpl();
  ...
  public interface NullifyFactory {
    ...
    RelNode createNullify(RelNode input, RexNode predicate,
        List<? extends RexNode> attributes, Set<CorrelationId> variablesSet);
  }
  ...
  private static class NullifyFactoryImpl implements NullifyFactory {
    public RelNode createNullify(final RelNode input, final RexNode predicate,
        final List<? extends RexNode> attributes, final Set<CorrelationId> variablesSet) {
      return LogicalNullify.create(input, predicate, attributes, ImmutableSet.copyOf(variablesSet));
    }
  }

  /**
   * Can create a {@link NullifyStar} of the appropriate type
   * for this rule's calling convention.
   */
  public interface NullifyStarFactory {
    /**
     * Creates a nullification star operator.
     */
    RelNode createNullifyStar(RelNode input, RexNode predicate,
        List<? extends RexNode> attributesA, List<? extends RexNode> attributesB,
        Set<CorrelationId> variablesSet);
  }

  /**
   * Implementation of {@link RelFactories.NullifyStarFactory} that
   * returns a vanilla {@link NullifyStar}.
   */
  private static class NullifyStarFactoryImpl implements NullifyStarFactory {
    public RelNode createNullifyStar(final RelNode input, final RexNode predicate,
        final List<? extends RexNode> attributesA, final List<? extends RexNode> attributesB,
        final Set<CorrelationId> variablesSet) {
      return LogicalNullifyStar.create(input, predicate, attributesA, attributesB,
          ImmutableSet.copyOf(variablesSet));
    }
  }
