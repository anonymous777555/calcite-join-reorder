...
public class JoinProjectTransposeRule extends RelOptRule {
  ...
  public JoinProjectTransposeRule(RelOptRuleOperand operand,
      String description, boolean includeOuter, RelBuilderFactory relBuilderFactory) {
  ...
  @Deprecated // to be removed before 2.0
  public JoinProjectTransposeRule(RelOptRuleOperand operand,
      String description, boolean includeOuter, ProjectFactory projectFactory) {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    // Checks if the rule allows outer joins or the join does not generate nulls on left side.
    final Project leftProj;
    ...
    // Checks if the rule allows outer joins or the join does not generate nulls on right side.
    final Project rightProj;
    ...
    // Returns if neither side has projection as direct child of the join.
    if (leftProj == null && rightProj == null) {
      return;
    }

    // Checks whether all projected attributes are strong.
    if (includeOuter) {
    ...
    final int nProjExprs = joinRel.getRowType().getFieldCount();
    final List<Pair<RexNode, String>> projects = new ArrayList<>();
    final RexBuilder rexBuilder = joinRel.getCluster().getRexBuilder();

    // Inserts the input fields from both left and right sides.
    createProjectExprs(
        leftProj,
        leftJoinChild,
        0,
    ...
    final List<RelDataType> projTypes = projects.stream()
        .map(pair -> pair.left.getType()).collect(Collectors.toList());
    final RelDataType projRowType =
        rexBuilder.getTypeFactory().createStructType(projTypes, Pair.right(projects));

    // create the RexPrograms and merge them
    final RexProgram bottomProgram = RexProgram.create(
        joinChildrenRowType,
        Pair.left(projects),
        null,
        projRowType,
    ...
    for (int i = 0; i < nProjExprs; i++) {
      ...
      if (joinType.isOuterJoin()) {
        final RelOptUtil.RexInputConverter converter = new RelOptUtil.RexInputConverter(
            rexBuilder,
            joinChildrenRowType.getFieldList(),
            newJoinFields,
            adjustments);
        newExpr = newExpr.accept(converter);
      }
  ...
  protected void createProjectExprs(
    ...
    final List<RelDataTypeField> childFields = joinChild.getRowType().getFieldList();

    // Creates reference to the inputs only if there is no projection.
    if (projRel == null) {
      for (int i = 0; i < childFields.size(); i++) {
        final RelDataTypeField field = childFields.get(i);
        final RexNode node = rexBuilder.makeInputRef(field.getType(), i + adjustmentAmount);
        projects.add(Pair.of(node, field.getName()));
      }
      return;
    }

    final int[] adjustments = new int[childFields.size()];
    Arrays.fill(adjustments, adjustmentAmount);
    for (final Pair<RexNode, String> pair: projRel.getNamedProjects()) {
      RexNode node = pair.left;

      // Shifts the referencing index if necessary.
      ...
      projects.add(Pair.of(node, pair.right));
    }
  }
}
