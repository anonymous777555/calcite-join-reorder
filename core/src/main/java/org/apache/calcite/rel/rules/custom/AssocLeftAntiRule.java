...
/**
 * AssocLeftAntiRule applies limited associativity on left join and anti join.
 *
 * Rule 11.
 */
public class AssocLeftAntiRule extends RelOptRule {
  //~ Static fields/initializers ---------------------------------------------
  private static final Logger LOGGER = CalciteTrace.getPlannerTracer();

  /** Instance of the current rule. */
  public static final AssocLeftAntiRule INSTANCE = new AssocLeftAntiRule(
      operand(Join.class,
  ...
  public AssocLeftAntiRule(RelOptRuleOperand operand,
      String description, RelBuilderFactory relBuilderFactory) {
    super(operand, relBuilderFactory, description);
  }

  public AssocLeftAntiRule(RelOptRuleOperand operand, String description) {
  ...
  @Override public void onMatch(final RelOptRuleCall call) {
    ...
    final List<RelDataTypeField> topRightFields = newTopJoin.getRowType().getFieldList()
        .subList(total - topJoinRight, total);
    final List<RexNode> topRightList = topRightFields.stream()
    ...
    final RelNode transformedNode = call.builder().push(newTopJoin)
        .gamma(topRightList).project(bottomList)
        .build();
    call.transformTo(transformedNode);
  }
}

// End AssocLeftAntiRule.java
