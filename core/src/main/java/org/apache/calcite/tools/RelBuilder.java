...
public class RelBuilder {
  ...
  private final RelFactories.NullifyStarFactory nullifyStarFactory;
  ...
  protected RelBuilder(Context context, RelOptCluster cluster,
    ...
    this.nullifyStarFactory =
        Util.first(context.unwrap(RelFactories.NullifyStarFactory.class),
            RelFactories.DEFAULT_NULLIFY_STAR_FACTORY);
  ...
  public RelBuilder gamma(Iterable<? extends RexNode> nodes) {
    final RelNode input = stack.peek().rel;

    // Constructs the nullification predicate.
    final List<RexNode> nodeList = Lists.newArrayList(nodes);
    final List<RexNode> checkNullList = nodeList.stream()
        .map(node -> call(SqlStdOperatorTable.IS_NULL, node))
        .collect(Collectors.toList());
    final RexNode nullifyPredicate = or(checkNullList);

    // Constructs the nullification attributes.
    final List<RelDataTypeField> fields = input.getRowType().getFieldList();
    final List<RexNode> nullifyAttributes = fields.stream()
        .map(field -> new RexInputRef(field.getIndex(), field.getType()))
        .collect(Collectors.toList());

    return nullify(nullifyPredicate, nullifyAttributes).bestMatch();
  }

  public RelBuilder gammaStar(Iterable<? extends RexNode> nodes,
      Iterable<? extends RexNode> exclude) {
    final RelNode input = stack.peek().rel;
    ...
    // Constructs the nullification attributes, while excluding some fields.
    final List<RelDataTypeField> fields = input.getRowType().getFieldList();
    final Set<RexNode> excludeFields = ImmutableSet.copyOf(exclude);
    final List<RexNode> nullifyAttributes = fields.stream()
        .map(field -> new RexInputRef(field.getIndex(), field.getType()))
  ...
  public RelBuilder nullify(RexNode predicate, Iterable<? extends RexNode> nodes) {
    return nullify(ImmutableSet.of(), predicate, nodes);
  }

  public RelBuilder nullify(Set<CorrelationId> variablesSet, RexNode predicate,
      Iterable<? extends RexNode> attributes) {
    final Frame frame = stack.pop();
    final List<? extends RexNode> nodesList = Lists.newArrayList(attributes);

    final RelNode nullify = nullifyFactory.createNullify(frame.rel, predicate,
        nodesList, variablesSet);
  ...
  public RelBuilder nullifyStar(RexNode predicate,
      Iterable<? extends RexNode> attributesA, Iterable<? extends RexNode> attributesB) {
    return nullifyStar(ImmutableSet.of(), predicate, attributesA, attributesB);
  }

  public RelBuilder nullifyStar(Set<CorrelationId> variablesSet, RexNode predicate,
      Iterable<? extends RexNode> attributesA, Iterable<? extends RexNode> attributesB) {
    final Frame frame = stack.pop();
    final List<? extends RexNode> listA = Lists.newArrayList(attributesA);
    final List<? extends RexNode> listB = Lists.newArrayList(attributesB);

    final RelNode nullifyStar = nullifyStarFactory.createNullifyStar(frame.rel, predicate,
        listA, listB, variablesSet);
    stack.push(new Frame(nullifyStar, frame.fields));
    return this;
  }
